    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId){
            ...
            R.id.menuDetailDelete -> {
                ...
                val builder = AlertDialog.Builder(binding.ivImage.context)
                ...
                GameProvider.gameList.forEach {
                    if(it.id == game?.id){
                        Log.i("RemoveDetail", "${game?.id} == ${it.id}")
                        position = i
                    }
                    i++
                }
                position?.let { GameProvider.gameList.removeAt(it) }
    ...
    private fun getImage(){
        Glide.with(binding.ivImage.context).load(game?.image).error(
            ...
            override fun onLoadFailed(
                ...
                Snackbar.make(binding.root, "Error al cargar algunas de las imágenes", Snackbar.LENGTH_SHORT).show()
                return false
            }
